...
```
::: list-danger 最后我们执行 `npm run build` 会报错
  - :label: 报错信息: `You may need an appropriate loader to handle thiss file type, currently no loaders`
  - :label: 解释：解析失败, 你需要合适**loader来处理这个文件类型**
:::
...
-  上面的错误信息告诉我们需要一个`loader`来加载这个 `css文件`，但是 `loader` 是什么呢？
   - :label: `loader` 可以用于对模块的源代码进行转换
   - :label:  我们可以将`css文件`也看成是一个模块,我们是通过`import`来**加载模块**将他**放入`webpack`依赖图中** 
...
::: list-tip css-loader是什么?
   - :label: 对于加载 `css文件` 来说，我们需要一个可以读取 `css文件` 的 `loader`；
   - :label: 这个loader最常用的是`css-loader` 
...
:::list-default 为什么用配置文件方式?
  - :label: 在配置文件中我们可以使用 `module.rules`中允许**我们配置多个loader**  
  - :label: 我们也可以继续使用其他的`loader`，**来完成其他文件的加载**
  - :label: 配置的方式能更好的展示loader的配置也,方便维护同时,也让你对各个`Loader`有一个全局的概览；
:::
#### 5.rules 配置属性
 ::: item-info module.rules的配置如下：

-   :microscope:  `rules`属性对应的值是一个数组：**`[Rule]`** 数组中存放的是一个个的`Rule`
-  :microscope: `Rule`是一个对象，对象中可以**设置多个属性**
    - :telescope: **`test` 属性**：用于对`resource`(资源)进行匹配的，通常会设置成正则表达式；
    - :telescope: **`use` 属性**：对应的值时一个数组：`[UseEntry]`
      ::: list-default UseEntry是什么
       - :label: `UseEntry`是一个对象，可以通过对象的属性来设置一些其他属性
         - :bookmark:`loader`：必须有一个loader属性，**对应的值是一个字符串；** 
         - :bookmark: `options`：可选的属性，**值是一个字符串或者对象**，值会被传入到loader中；
         - :bookmark: `query`：**目前已经使用options来替代** 
      :::
  - :telescope: **loader属性**：是`use`:` [ { loader } ] `的简写。
  - :telescope: **include** 指定 **需要处理的目录**
  - :telescope: **exclude** 排除 **不需要处理的目录或文件**
  - :telescope: [更多配置查看webpack官网](https://webpack.js.org/configuration/module/#modulerules)
 :::
- 在配置文件中，添加`css-loader`进行解析css
  ...
  :bookmark:  现在我们已经可以通过 `css-loader` **来加载css文件了** <br/>
  :bookmark: 此时我们运行`npm run build`,虽然没有报错也把类名添加上去了，但是**css效果并没有生效**  <br/>
...
::: list-warning css没有生效的原因?
   -  :pushpin: 因为 `css-loader` 只是负责 `.css文件进行解析` **并不会将解析之后的css插入到页面中；**
   -  :pushpin:   如果我们希望再完成插入style的操作,那么我们 **还需要另外一个loader** `style-loader`
:::
...
::: list-default 在配置文件中,添加style-loader
   -  :pushpin: 注意事项：`webpack`**在执行loader的时候，是通过从后往前执行的**
...
<!-- - 当我们在项目开发中一般不会直接使用css去开发项目，而是使用css的预处理器进行快速开发 <p></p>
    :pushpin:  常用的css预处理器 `less` `scss` `stylus` <p></p>
    :pushpin:  在css目录创建 `index.less` `index.scss` `index.stylus` <p></p>
    :pushpin:   我们编写如下的 `less` 样式  <p></p> -->
- 当我们在项目开发中一般**不会直接使用css去开发项目**，而是使用 **css的预处理器进行快速开发**
::: list-info 处理器
...
:::
 :pushpin: 编写 `less` 样式 
  ...
  :pushpin: 每个 `css预处理器` 都有自己单独语法，**浏览器不认识这些语法** <br/>
  :pushpin: 通过 `webpack` 打包进行 **转换为浏览器认识的css语法** <br/>
  :pushpin: 每个`css预处理器` 都有合适的 `loader` 进行转换为`css`  
...
#### 2.安装各自对应的loader
 ...
 :pushpin:  `Less`、`Scss` `Stylus`工具 <br/>
 :pushpin: 直接用工具命令将预处理器语法 **转换为普通css**
...
#### 3.配置预处理器
 ...
 :pushpin:  如果按照上面这种方式去**转换成普通css**,非常麻烦,如何**自动给我转换普通css**呢? <br/>
 :pushpin: 这个我们就要在配置文件去添加对应的 `loader` 处理 <p></p>
 :pushpin:  在们之前的`components` 文件下 `cps.js`  添加以下代码 
```js [cps.js]
import '../css/index.css';
...
- 以下是配置文件解释
 ...
 :pushpin: 使以下是配置文件解释：
::: list-info 解释
- :pushpin: 我们先让各自对应loader `less-loader` `scss-loader` `stylus-loader`将对应代码转换成普通css代码
-  :pushpin:  然后通过`css-loader` 再次解析，交给 `style-loader `将样式添加到页面中
:::
...
- 什么是PostCSS呢？
::: list-success 解释
- :pushpin:  `PostCSS` 是一个通过 `JavaScript` 来 **转换样式的工具**
-  :pushpin:  这个工具可以帮助我们进行一些CSS的转换和适配 比如 **自动添加浏览器前缀、css样式的重置**
-
:::
  - 在  `index.css`  添加以下样式
...
- :pushpin:  第一步：查找PostCSS在构建工具中的扩展，**比如webpack中的postcss-loader；**
-  :pushpin:  第二步：选择可以添加你需要的PostCSS相关的插件
:::
...
- 我们修改加载 `css` 的 `loader`:（配置项已经过多，给出一部分了）
...
- 如何使用呢？有两种配置方式
  ...
  - 第一种方式 写在`webpack.config.js` 对应 `loader` 里面 `options` 中
...
- :pushpin: 创建一个`postcss.config.js`将`options`里面的配置全部放到这个文件里面
- :pushpin: 这样能减少 `webpack` 配置文件里面东西，找配置项也比较好找
-  :pushpin:  在 `postcss.config.js`中, 可以减少`postcssOptions`这一层**对象嵌套**
:::
...
- :microscope:  它是一个postcss**功能强大的插件工具**，能够将现代 CSS 语法转换 **为兼容性更强的代码**，并自动处理**浏览器适配问题**。
- :microscope:  它的功能有哪些呢？
   - :satellite: **自动添加浏览器前缀** 它内部已经帮我集成了`autoprefiexr`  所以我们就不用单独下载配置
   -  :satellite: **支持未来 CSS 语法转换**
   - :satellite: **将尚未被广泛支持的 CSS 新特性**（如**嵌套、变量、颜色函数**等）转换为兼容性代码
...
- :microscope:  **自定义属性和变量**
  ...
  - :microscope:  **数学计算支持**
    ...
    ```
 - :petri_dish:  总结：**postcss-preset-env 本质上是通过整合多个 PostCSS 插件，让开发者能够使用最新的 CSS 特性，而不必担心浏览器兼容性问题。** 更多配置和插件集成可参考[官方文档](https://github.com/csstools/postcss-preset-env)。
...
- :gear: 这样我们`npm run build`  查看效果图 这两个功能它都实现了
